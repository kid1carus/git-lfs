...
// FindWorktree returns the git config value in worktree or local scope for the key, depending on whether extensions.worktreeConfig is present
...
// SetWorktree sets the git config value for the key in the worktree or local config, depending on whether extensions.worktreeConfig is present
...
// UnsetLocalSection removes the entire named section from the localconfig
func (c *Configuration) UnsetLocalSection(key string) (string, error) {
	return c.gitConfigWrite("--local", "--remove-section", key)
}

// UnsetWorktreeSection removes the entire named section from the worktree or local config, depending on whether extensions.worktreeConfig is present
